...
namespace _4th_Lab
{
    class Theory
    {
        ...
        static int ex_8()
        {
            ...
            int[,] matrix14 = new int[rows, cols];
            ...
            for(int i = 0; i < rows; i++)
            {
                ...
                for(int j = 0; j < cols; j++)
                {
                    if (matrix13[i, j] > 0) counterOfPositive++;
                }
                tuple = (indexOfRow, counterOfPositive);
                list.Add(tuple);
                counterOfPositive = 0;
            }
            ...
            for(int i = 0; i < rows; i++)
            {
                for(int j = 0; j < cols; j++)
                {
                    matrix14[i, j] = matrix13[list[i].Item1, j];
                }
            }
            Console.WriteLine("Lvl_3 || ex.8: ");
            for (int i = 0; i < rows; i++)
            {
                for(int j = 0; j < cols; j++)
                {
