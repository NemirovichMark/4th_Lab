...
namespace _4th_Lab
{
    class Theory
    {
        ...
        static int ex_11()
        {
            ...
            int[] listOfZeroInRows = new int[rows];
            int zeroCounter = 0;
            for(int i = 0; i < rows; i++)
            {
                for (int j = 0; j < cols; j++)
                {
                    if (matrix13[i, j] == 0)
                    { 
                        listOfZeroInRows[i] = 0; 
                        zeroCounter++;
                        break;
                    }
                    else listOfZeroInRows[i] = i + 1;
                }
            }
            int newLength = rows - zeroCounter;
            int[,] matrix14 = new int[newLength, cols];
            int k = 0;
            for(int i = 0; i < rows; i++)
            {
                if (listOfZeroInRows[i] == 0) continue;
                for (int j = 0; j < cols; j++)
                {
                    matrix14[k, j] = matrix13[i, j];
                }
                k++;
            }
            Console.WriteLine("Lvl_3 || ex.11: ");
            for (int i = 0; i < newLength; i++)
            {
                for(int j = 0; j < cols; j++)
                {
                    Console.Write(matrix14[i,j] + " ");
                }
                Console.WriteLine();
            }
            return 0;
