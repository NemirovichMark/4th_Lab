    ...
    class Theory
    {
        ...
        static int ex_8()
        {
            ...
            if (rows == 6 && cols == 6)
            {
                ...
                {
                    int buffer = matrix13[i, indexes[i]];
                    matrix13[i, indexes[i]] = matrix13[i + 1, indexes[i + 1]];
                    matrix13[i + 1, indexes[i + 1]] = buffer;
                }
                Console.WriteLine("Lvl_2 || ex.8: ");
                for (int i = 0; i < rows; i++)
                {
                    for (int j = 0; j < cols; j++)
                    {
            ...
            else Console.WriteLine("Task requires a matrix with 6x6 size");
            return 0;
            #endregion
        }
